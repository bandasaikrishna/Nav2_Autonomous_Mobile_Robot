...
odrive_endpoint::odrive_endpoint()
{
    if (libusb_init(&libusb_context_) != LIBUSB_SUCCESS) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"), "* Error initializing USB!");
    }
}
...
int odrive_endpoint::execFunc(int endpoint_id)
{
    ...
    if (status != LIBUSB_SUCCESS) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* execFunc: Error in endpoint request (%d)!", endpoint_id);
    }
    return status;
}
    	...
    	bool ack, int length, bool read, int address)
{
    ...
    if (result != LIBUSB_SUCCESS) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error in transfering data to USB!");
        ep_lock.unlock();
        return result;
    } else if (packet.size() != sent_bytes) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error in transfering data to USB, not all data transferred!");
    }
    ...
    if (ack) {
        ...
        if (result != LIBUSB_SUCCESS) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error in reading data from USB!");
            ep_lock.unlock();
            return result;
        }
        ...
        if (received_seq_no != seq_no) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error Received data out of order");
        }
...
int odrive_endpoint::init(uint64_t serialNumber)    	
{
    ...
    for (size_t i = 0; i < device_count; ++i) {
        ...
        if (result != LIBUSB_SUCCESS) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error getting device descriptor");
            continue;
        }
        ...
        if (desc.idVendor == ODRIVE_USB_VENDORID && desc.idProduct == ODRIVE_USB_PRODUCTID) {
            ...
            if (libusb_open(device, &device_handle) != LIBUSB_SUCCESS) {
                RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error opeening USB device");
                continue;
             }
            ...
            if ((libusb_kernel_driver_active(device_handle, ifNumber) != LIBUSB_SUCCESS) &&
                ...
                RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Driver error");
                libusb_close(device_handle);
                continue;
            }

            if ((result = libusb_claim_interface(device_handle, ifNumber)) !=  LIBUSB_SUCCESS) {
                RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error claiming device");
                libusb_close(device_handle);
                continue;
            } else {
 		...
 		result = libusb_get_string_descriptor_ascii(device_handle, desc.iSerialNumber, buf, 127);
                if (result <= 0) {
                    RCLCPP_ERROR(rclcpp::get_logger("odrive_endpoint"),"* Error getting data");
                    ...
                    if (sn.compare(0, strlen((const char*)buf), (const char*)buf) == 0) {
                        RCLCPP_INFO(rclcpp::get_logger("odrive_endpoint"),"Device 0x%8.8lX Found", serialNumber);
