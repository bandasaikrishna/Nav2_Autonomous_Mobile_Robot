...
int updateTargetConfig(odrive_endpoint *endpoint, Json::Value odrive_json, string config_file)
{
    ...
    if (cfg.is_open()) {
        ...
        if (!res) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error parsing %s json!", config_file.c_str());
            return ODRIVE_ERROR;
        }
        else {
            if (setChannelConfig(endpoint, odrive_json, config_json, false) != ODRIVE_OK) {
                RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error setting configuration!");
                return ODRIVE_ERROR;
            }
        }
        if (calibrateAxis0(endpoint, odrive_json) != ODRIVE_OK) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error calibrating axis 0!");
            return ODRIVE_ERROR;
        }
        if (calibrateAxis1(endpoint, odrive_json) != ODRIVE_OK) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error calibrating axis 1!");
            return ODRIVE_ERROR;
        }
        if (execOdriveFunc(endpoint, odrive_json, "save_configuration") != ODRIVE_OK) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error saving configuration!");
            return ODRIVE_ERROR;
        }
    }
    else {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error opening configuration file!");
    return ODRIVE_ERROR;
    }
}
    	...
    	bool save_config = 0)
{
    ...
    RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"Setting %s config value", name.c_str());
    ...
    if (!type.compare("float")) {
            ...
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error: invalid type for %s", name.c_str());
            return ODRIVE_ERROR;
        }
    }
...
int getJson(odrive_endpoint *endpoint, Json::Value *odrive_json)
{
    ...
    if (!res) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error parsing json!");
        return 1;
    }
    return 0;
}
...
int getObjectByName(Json::Value odrive_json, std::string name, odrive_object *odo)
{
    ...
    if (ret) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* %s not found!", name.c_str());
    }
    return ret;
}
    	...
    	std::string object, TT &value)
{
    ...
    if (ret) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error getting ID for %s", object.c_str());
    return ret;
    }

    if (odo.access.find("r") == string::npos) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error: invalid read access for %s", object.c_str());
        return ret;
    }

    if (!odo.type.compare("float")) {
    if (sizeof(value) != sizeof(float)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not float", object.c_str());
            return ODRIVE_ERROR;
    }
    }
    else if (!odo.type.compare("uint8")) {
        if (sizeof(value) != sizeof(uint8_t)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not uint8_t", object.c_str());
            return ODRIVE_ERROR;
        }
    }
    else if (!odo.type.compare("uint16")) {
        if (sizeof(value) != sizeof(uint16_t)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not uint16_t", object.c_str());
            return ODRIVE_ERROR;
        }
    }
    else if (!odo.type.compare("uint32")) {
        if (sizeof(value) != sizeof(uint32_t)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not uint32_t", object.c_str());
            return ODRIVE_ERROR;
        }
    }
    else if (!odo.type.compare("uint64")) {
        if (sizeof(value) != sizeof(uint64_t)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not uint64_t", object.c_str());
            return ODRIVE_ERROR;
        }
    }
    else if (!odo.type.compare("int32")) {
        if (sizeof(value) != sizeof(int)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not int", object.c_str());
            return ODRIVE_ERROR;
        }
    }
    else if (!odo.type.compare("int16")) {
        if (sizeof(value) != sizeof(short)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not short", object.c_str());
            return ODRIVE_ERROR;
        }
    }
    else if (!odo.type.compare("bool")) {
        if (sizeof(value) != sizeof(bool)) {
            RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error value for %s is not bool", object.c_str());
            return ODRIVE_ERROR;
        }
    }
    else {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error: invalid type for %s", object.c_str());
        return ODRIVE_ERROR;
    }
                ...
                std::string object, T &value)
{
    ...
    if (odo.access.find("w") == string::npos) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error: invalid write access for %s", object.c_str());
        return ODRIVE_ERROR;
    }
                ...
                std::string object)
{
    ...
    ret = getObjectByName(odrive_json, object, &odo);
    if (ret) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error getting ID");
        return ret;
    }

    if (odo.type.compare("function")) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error invalid type");
        return ret;
    }

    ret = endpoint->execFunc(odo.id);
    if (ret != LIBUSB_SUCCESS) {
        RCLCPP_ERROR(rclcpp::get_logger("odrive_util"),"* Error executing %s function", object.c_str());
    }
    return ret;
}
